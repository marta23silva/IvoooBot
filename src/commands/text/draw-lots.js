...
const slashcmd_msg = error_msg(`You have to use the slash command to use "draw-lots".`);
...
module.exports = {
	...
	.setName('draw-lots')
	.setDescription('Assigns one item of the first list to another one of the second list.')
	...
	async execute(interaction, tokens) {
		...
		if(list1.length !== list2.length) { return interaction.reply({ embeds: [ length_msg ] }); }
		
		for(let i = 0; i < list1.length; i++) { shuffled.push('**' + list1[i] + '** - ' + list2[i]); }
