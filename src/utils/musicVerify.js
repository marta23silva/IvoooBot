...
module.exports = {
	...
	verifyChannel: (message, player) => {
		...
		if(player && message.member.voice.channel != player.voiceChannel) return message.channel.send(embed.embed_yellow_warning('❗️ You need to be in the same voice channel as Ivooo to use this command.'));
	}
}