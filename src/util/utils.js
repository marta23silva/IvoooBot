...
const no_player = warning_msg(`Try to play some music, or inviting me to a voice channel first.`);
...
function verifyChannel(interaction, player) {
  ...
  if (!player) {
    if (interaction.commandName!=='join' && interaction.commandName!=='play')
      return interaction.reply({ embeds: [no_player] });
  } else {
    if (interaction.member.voice.channel != player.voiceChannel)
